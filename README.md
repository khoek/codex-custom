...
```text
codex+k (CODEX_UUID_WHICH_YOU_CAN_USE_TO_RESUME): quota exceeded {"version":1,"resume_args":["--model","MODEL","-c","model_provider=\"PROVIDER\"","-c","service_tier=\"TIER\"","-c","model_reasoning_effort=\"EFFORT\""]}
```

The UUID is the primary session ID accepted by `codex resume`. The compact,
versioned JSON suffix contains argv tokens generated by Codex for restoring the
primary session's exact model, model provider, service tier, and reasoning
effort; the reasoning override is omitted when the session has none. An
implicit ordinary tier is emitted explicitly as `default`, so a changed config
cannot silently turn a resumed run into a fast-tier run. Retryable errors,
